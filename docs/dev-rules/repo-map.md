...
| `anthropic-responses-bridge` | `anthropic-compat-proxy` 的进程内协议转换处理器：作为 `RoutingDecision.localHandler` 完成 Anthropic Messages API ↔ OpenAI Responses API 转换，不独立监听端口 | desktop |
